...
`git clone https://github.com/ByteLegendQuest/java-fix-method-return-npe`

A project directory named `java-fix-method-return-npe` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-fix-method-return-npe` directory which is created just now.
